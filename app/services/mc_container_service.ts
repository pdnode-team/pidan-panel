...
export default class McContainerService {
  ...
  async startContainer(server: McServer): Promise<void> {
    ...
    // 2. Check server executable jar (jail the configured name to a bare file)
    const jarFileName = McJarsService.sanitizeJarFileName(server.serverJar)
    const jarPath = join(server.dataDirectory, jarFileName)
    try {
      await access(jarPath, constants.F_OK)
    } catch {
      throw new Error(
        `Server jar file "${jarFileName}" not found in server directory. Please upload it first.`
      )
    }

    // 3. Ensure EULA agreement (only write when missing so operator edits survive)
  ...
  async removeContainer(server: McServer): Promise<void> {
    ...
    let scanFailed: Error | null = null

    const matchesServer = (info: {
      Names?: string[]
      ...
      const matchesPort =
        names.some((n) => n.includes(server.identifier) || n.startsWith('/pidan')) &&
    ...
    try {
      ...
      scanFailed = scanErr
      logger.warn(`Docker container cleanup scan error: ${scanErr?.message}`)
    }

    // 3. Verify nothing associated with this instance remains; abort deletion otherwise
    let lingering: unknown[] = []
    try {
      const allContainers = await this.docker.listContainers({ all: true })
      lingering = allContainers.filter((info) => !removedIds.has(info.Id) && matchesServer(info))
    } catch (verifyErr: any) {
      throw new Error(
        `Docker engine unavailable; cannot verify container removal for server ${server.id}: ${verifyErr?.message}`
      )
    }

    if (scanFailed || lingering.length > 0) {
      const reason = scanFailed?.message || `${lingering.length} container(s) still associated`
      throw new Error(
        `Failed to remove containers for server ${server.id} (${reason}); deletion aborted.`
      )
    }
  }
}
